...
def fetch_articles(config_file_path):
    ...
    continuation = None
    file_path = os.path.expanduser('~/CVE_JSON.txt')
    all_vulnerabilities = []
...
def batch_cves_and_call_api(config_file_path, cve_ids):
    ...
    config.read(config_file_path)
    api_key = config['FEEDLY']['ApiKey']
    insights_path = os.path.expanduser(config['FEEDLY'].get('InsightsFilePath', '~/CVE_INSIGHTS.txt'))
    batch_size = 100
...
if __name__ == '__main__':
    config_file_path = '/Users/aomaley/CVE_AUTOMATION.ini' #Replace with CVE_AUTOMATION File path
